...
describe('AllPerks page (Directory)', () => {
  ...
  test('lists public perks and responds to merchant filtering', async () => {
    ...
    // Find and interact with the merchant filter dropdown (native <select>)
    const merchantFilter = screen.getByRole('combobox');
    fireEvent.change(merchantFilter, { target: { value: seededPerk.merchant } });

    // Verify the filtered results
    await waitFor(() => {
      // The perk with the selected merchant should be visible
      const perkElement = screen.getByText(seededPerk.title);
      expect(perkElement).toBeInTheDocument();
      
      // Verify the summary text reflects the filtered results
